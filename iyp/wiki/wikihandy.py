...
# DEFAULT_WIKI_SPARQL = 'http://localhost:8989/bigdata/namespace/wdq/sparql'
# 'https://exp1.iijlab.net/wdqs/bigdata/namespace/wdq/sparql'
...
class Wikihandy(object):
    ...
    def __dict2quantity(self, target):
        """Transform a dictionnary representing a quantity (value, unit: a QID to the
        item representing the unit, lowerBound, upperBound) to a wikibase quantity
        object."""
    ...
    def _update_statement_local(self, claims, target, ref_urls=None, sources=None):
        ...
        If a reference URL is given, then it will update the statement that have the
        same reference URL. Otherwise it will update the first statement that has no
        reference URL.
        """
    ...
    def upsert_statements(self, summary, item_id, statements, commit=True,
                          ...
                          asynchronous=True):
        """Update statements that have the same reference URLs or create new statements.
        All of this in only one or two API calls.
    ...
    def add_statement(self, summary, item_id, property_id, target, qualifiers=[]):
        """Create new claim, if the property datatype is 'wikibase-item' then the target
        is expected to be the item PID.

        For properties with a different datatype the value of target is used as is.
    ...
    def get_qid(self, label, create=None):
        """Retrieve item id based on the given label. Returns None if the label is
        unknown or create it with values passed in the create parameter.
    ...
    def prefix2qid(self, prefix, create=False):
        ...
        if len(self._prefix2qid) == 0:
            ...
            """ % (
                    self.get_pid('instance of'),
                    self.get_qid('IP routing prefix'),
            )
    ...
    def country2qid(self, cc, create=True):
        """Find a country QID or add the country to wikibase if it doesn't exist and
    ...
    def id4alllabels(self):
        """Return two dictionaries, one for properties and  one for items, with labels
        as keys and Q/P IDs as values. For entities that have the same label only the
...
def select_claims(claims_dict, ref_pid, ref_values):
    """Select claims from claims_dict that have a reference statement ref_pid with one
    of the values given in ref_values."""
...
def select_first_claim(claims_list, ref_pid, ref_values):
    """Select first claim from all_claims that have a reference statement ref_pid with
    one of the values given in ref_values.

    Return None if such claim is not found.
    """
...
def unpack_statement(statement):
    """Return four objects (pid, target, references, qualifiers) regardless of the
    length of the given statement."""
