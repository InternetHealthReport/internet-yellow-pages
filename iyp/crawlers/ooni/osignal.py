...
class Crawler(OoniCrawler):
    ...
    def process_one_line(self, one_line):
        ...
        if super().process_one_line(one_line):
            return
        signal_backend_status = one_line['test_keys']['signal_backend_status']
        if signal_backend_status is None:
    ...
    def batch_add_to_iyp(self):
        ...
        signal_id = self.iyp.get_node('Tag', {'label': label}, create=True)
        ...
        for (asn, country), result_dict in self.all_percentages.items():
            ...
            censored_links.append(
                {'src_id': asn_id, 'dst_id': signal_id, 'props': [props, self.reference]}
            )
