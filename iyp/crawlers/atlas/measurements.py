...
class Crawler(object):
    def __init__(self):
        ...
        self.msmid_qid = self.wh.get_qid(MSMID_LABEL,
                                         create={  # Create it if it doesn't exist
                                             # Commit message
                                             'summary': 'add RIPE Atlas measurements',
                                             # Description
                                             'description': 'Identifier for a measurement in the RIPE Atlas platform'
                                         })
    ...
    def update_msm(self, msm):
        ...
        if 'protocol' in msm and msm['protocol']:
            ...
            if protocol is None:
                logging.error("protocol unknown: msm['protocol']")
                print("protocol unknown: msm['protocol']")
            else:
        ...
        # Measurement target
        if msm['target']:
            statements.append([self.wh.get_pid('target'), self.target_qid(
                msm['target'], msm['target_prefix']), self.reference])

        if not msm['resolve_on_probe'] and msm['target_ip']:
            statements.append([self.wh.get_pid('target'), self.target_qid(
                msm['target_ip'], msm['target_prefix']), self.reference])
        ...
        # Commit to wikibase
        # Get the measurement QID (create if measurement is not yet registered) and
        # commit changes
    ...
    def msm_qid(self, msm):
        """Find the measurement QID for the given probe ID. If this measurement is not
        yet registered in the wikibase then add it.
