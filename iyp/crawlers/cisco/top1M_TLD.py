...
URL = 'https://s3-us-west-1.amazonaws.com/umbrella-static/top-1m-TLD.csv.zip'
ORG = 'CISCO'
NAME = 'umbrella.top1m_tld'
...
class Crawler(BaseCrawler):
    ...
    def run(self):
        ...
        self.cisco_qid = self.iyp.get_node('Ranking', {'name': 'CISCO Umbrella Top 1 million TLD'}, create=True)

        sys.stderr.write('Downloading latest list...\n')
        req = requests.get(URL)
        if req.status_code != 200:
            sys.exit('Error while fetching Cisco Umbrella Top 1M TLD csv file')

        links = []
        domains = set()
        # open zip file and read top list
        with ZipFile(io.BytesIO(req.content)) as z:
            with z.open('top-1m-TLD.csv') as list:
